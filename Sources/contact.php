...
if ($helpdesk_enable == "yes") {
	if ($input->p['do'] == "checkticket") {
		if (verifyToken("check_ticket", $input->p['token']) !== true) {
			serveranswer(0, $lang['txt']['invalidtoken']);
		}

		$verifyticket = $db->fetchOne("SELECT COUNT(*) AS NUM FROM helpdesk_ticket WHERE ticket='" . $db->real_escape_string($input->pc['ticketid']) . "'");
	...
	if ($input->p['action'] == "open") {
		...
		if (verifyToken("new_ticket", $input->p['token']) !== true) {
			serveranswer(0, $lang['txt']['invalidtoken']);
		}
		...
		if ((empty($subject) || !is_numeric($department)) || empty($message)) {
			...
			$verifydpto = $db->fetchOne("SELECT COUNT(*) AS NUM FROM helpdesk_department WHERE id=" . $db->real_escape_string($department));
