...
if ($input->r['do'] == "search" && $input->r['search_word']) {
	if (verifyToken("forum_search", $input->p['token']) !== true) {
		header("location: ./forum.php?page=search");
		exit();
	}
				...
				case "searchtype":
					if ($input->r['searchtype'] == 1) {
						if ($input->r['subjectonly'] == 1) {
							$cond .= "title LIKE '%" . $db->real_escape_string($input->r['search_word']) . "%' AND id=topic_rel AND ";
						}
						else {
							$cond .= "message LIKE '%" . $db->real_escape_string($input->r['search_word']) . "%' OR (title LIKE '%" . $db->real_escape_string($input->r['search_word']) . "%' AND id=topic_rel) AND ";
						}
					}
					else {
						...
						foreach ($vars as $v) {
							...
							if ($input->r['subjectonly'] == 1) {
								$condb .= "(title LIKE '%" . $db->real_escape_string($v) . "%' AND id=topic_rel) OR ";
								continue;
							}

							$condb .= "(message LIKE '%" . $v . "%' OR (title LIKE '%" . $db->real_escape_string($v) . "%' AND id=topic_rel)) OR ";
						}
