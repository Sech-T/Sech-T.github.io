...
if (is_numeric($input->g['read'])) {
	...
	$msgid = $db->real_escape_string($input->gc['read']);
...
if ($input->p['do'] == "send") {
	...
	if (is_numeric($input->pc['user_to_id'])) {
		$verify = $db->fetchOne("SELECT COUNT(*) AS NUM FROM members WHERE id='" . $db->real_escape_string($input->pc['user_to_id']) . "' AND personal_msg='yes'");
		...
		if (!empty($input->pc['user_to'])) {
			$verify = $db->fetchOne("SELECT COUNT(*) AS NUM FROM members WHERE username='" . $db->real_escape_string($input->pc['user_to']) . "' AND personal_msg='yes'");

			if ($verify != 0) {
				$user_to = $db->fetchOne("SELECT id FROM members WHERE username='" . $db->real_escape_string($input->pc['user_to']) . "'");
...
if (is_numeric($input->g['reply']) || is_numeric($input->g['quote'])) {
	...
	$msgid = $db->real_escape_string($msgid);
	$verify = $db->fetchOne("SELECT COUNT(*) AS NUM FROM messages WHERE id='" . $msgid . "' AND user_to=" . $user_info['id']);
...
if (is_numeric($input->g['to'])) {
	$usrid = $db->real_escape_string($input->gc['to']);
