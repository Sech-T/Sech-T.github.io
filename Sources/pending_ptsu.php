...
if (is_numeric($_REQUEST['id'])) {
	$ptsu_id = $db->real_escape_string(cleanfrm($_REQUEST['id']));
	$verify = $db->fetchOne("SELECT COUNT(*) AS NUM FROM ptsu_offers WHERE id=" . $ptsu_id . " AND user_id=" . $user_info['id']);

	if ($_REQUEST['a'] == "accept") {
		verifyajax();
		$request_id = $db->real_escape_string(cleanfrm($_POST['rid']));
		$verify_req = $db->fetchOne("SELECT COUNT(*) AS NUM FROM ptsu_requests WHERE id='" . $request_id . "' AND ptsu_id=" . $ptsu_id . " AND status='Pending'");
