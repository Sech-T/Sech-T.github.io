	...
	if (!empty($input->p['action']) && is_array($input->p['ref'])) {
		...
		if ($input->p['action'] == "recycle") {
			...
			foreach ($_POST['ref'] as $i => $refid) {
				$verifyuser = $db->fetchOne("SELECT COUNT(*) AS NUM FROM members WHERE rented=" . $user_info['id'] . " AND id=" . $db->real_escape_string($refid));

				if ($verifyuser != 0) {
					recycle($user_info['id'], $refid);
			...
			if (is_numeric($_POST['action'])) {
				...
				$verify = $db->fetchOne("SELECT COUNT(*) AS NUM FROM rent_discount WHERE id=" . $db->real_escape_string($discountid));

				if ($verify == 0) {
					serveranswer(0, $lang['txt']['selectdaysextend']);
				}

				$discount = $db->fetchRow("SELECT * FROM rent_discount WHERE id=" . $db->real_escape_string($discountid));
				...
				foreach ($input->p['ref'] as $i => $refid) {
					...
					if ($verifyuser != 0) {
						$rented_expires = $db->fetchOne("SELECT rented_expires FROM members WHERE id=" . $db->real_escape_string($refid));
