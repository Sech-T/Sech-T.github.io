...
<div class=\"site_title\">Manage PTSU Pending Applications</div>
...
echo " />
            ...
            ";
$statusvalues = array("Pending" => "Pending review by advertiser", "Rejected1" => "Rejected - Pending review by admin", "Pending2" => "All pending review by admin");
...
echo "\">
  <table width=\"100%\" class=\"widget-tbl\">
  	<tr class=\"titles\">
        ...
        <td>";
echo $paginator->linkorder("ptsu_id", "PTSU ID");
echo "</td>
        <td>";
...
echo "</td>
        <td align=\"center\">";
echo $paginator->linkorder("owner_id", "Advertiser");
echo "</td>
        <td align=\"center\">";
echo $paginator->linkorder("user_id ", "Applicant");
echo "</td>
        <td align=\"center\">";
echo $paginator->linkorder("value ", "Value");
echo "</td>
        <td align=\"center\">";
...
while ($r = $db->fetch_array($q)) {
	$tr = ($tr == "tr1" ? "tr2" : "tr1");
	$owner = ($r['owner_id'] == 0 ? "<span style=\"color:green\">Administrator</span>" : "<a href=\"./?view=members&edit=" . $r['owner_id'] . "\">" . $db->fetchOne("SELECT username FROM members WHERE id=" . $r['owner_id']) . "</a>");
	...
	switch ($r['status']) {
		case "Pending":
			if ($r['owner_id'] != 0) {
				echo "<span style=\"color:orange\">" . $r['status'] . " review by advertiser</span>";
			}
			else {
				echo "<span style=\"color:#996600\">" . $r['status'] . " review by admin</span>";
			}
			break;

		case "Rejected1":
			echo "<span style=\"color:red\">Rejected - Pending review by admin</span>";
			break;
...
if ($paginator->totalResults() == 0) {
	echo "    <tr>
    	<td colspan=\"9\" align=\"center\">Records not found</td>
    </tr>
    ";
}
