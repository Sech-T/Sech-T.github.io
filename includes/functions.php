...
function showrotatingbannersdays() {
	global $db;
	global $settings;

	if ($settings['banneradsdays_available'] == "yes") {
		$todayis = time();
		$res = $db->fetchOne("SELECT COUNT(*) AS NUM FROM bannerday_ads WHERE status='Active' AND expires>" . $todayis);

		if ($res != 0) {
			$banner = $db->fetchRow("SELECT id, img, views from bannerday_ads WHERE status='Active' AND expires>" . $todayis . " ORDER BY RAND() LIMIT 0,1");
			$set = array("views" => $banner['views'] + 1);
			$upd = $db->update("bannerday_ads", $set, "id = " . $banner['id']);
			echo "<a href=\"bannerdayclick.php?id=" . $banner['id'] . "\" target=\"_blank\"><img src=\"" . $banner['img'] . "\" border=\"0\" width=\"468\" height=\"60\"></a>";
			return null;
		}
...
function showloginads() {
	...
	if ($settings['loginads_available'] == "yes") {
		...
		if ($res != 0) {
			$banner = $db->fetchRow("SELECT id, image, views FROM login_ads WHERE status='Active' AND expires>=" . $today . " ORDER BY RAND() LIMIT 0,1");
			$set = array("views" => $banner['views'] + 1);
			$upd = $db->update("login_ads", $set, "id = " . $banner['id']);
