...
function set_gateway(val){
	if(val != ''){
		if(val == 'balance'){
			$("#min_deposit").html('{$lang.txt.min_deposit}: ${$settings.amount_transfer}');
		}else{
			$("#min_deposit").html('{$lang.txt.min_deposit}: $'+gateway[val]);
		}
		$("#min_deposit").show();
	}else{
		$("#min_deposit").hide();
...
function complete_deposit(){
	...
	if(isNaN(parseFloat(amount))){
			$("#error_box").html('{$lang.txt.min_deposit}: ${$settings.amount_transfer}');
			$("#error_box").fadeIn();
			return false;
	}
	...
	if(gatewayid == 'balance'){
		...
		if(amount < gateway[gatewayid]){
			$("#error_box").html('{$lang.txt.min_deposit}: $'+gateway[gatewayid]);
			$("#error_box").fadeIn();
		}else{
